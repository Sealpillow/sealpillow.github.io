...
if (-not $SkipPipeline) {
    ...
    try { python main\run-analysis-pipeline.py }
    finally { Pop-Location }
    if (-not $?) { Write-Error "Pipeline failed — aborting deploy." }
...
$jsonFiles = Get-ChildItem $JsonSrc -Filter "*.json" |
             ...
             Sort-Object
$manifest  = [pscustomobject]@{ files = $jsonFiles } | ConvertTo-Json -Compress
$manifest | Out-File (Join-Path $JsonSrc "manifest.json") -Encoding utf8NoBOM
...
# ── 4. Patch dashboard.html → static mode ───────────────────────────────────
